...
설정에서는 전체 커서 배율을 50–200%로 조절하거나 실행 중 Dock 아이콘을 숨길 수 있습니다. Dock 아이콘을 숨기면 메뉴 막대의 Momiji 아이콘에서 앱 열기·설정·종료를 사용할 수 있습니다. **컴퓨터 로그인 시 Momiji 자동 실행**을 직접 켜면 Momiji가 macOS의 **로그인 시 열기** 목록에 등록되고, 로그인 직후 활성 테마를 적용합니다. 앱을 수동으로 열거나 절전 복귀·사용자 세션 활성화가 발생해도 활성 테마를 다시 적용합니다. 관리자 권한은 요구하지 않습니다.

자동 실행을 등록하려면 DMG 안에서 직접 실행하지 말고 Momiji를 `/Applications`에 복사해 실행해야 합니다. macOS에서 추가 승인을 요구하면 Momiji 설정의 **로그인 항목 설정 열기**를 눌러 허용하세요.
